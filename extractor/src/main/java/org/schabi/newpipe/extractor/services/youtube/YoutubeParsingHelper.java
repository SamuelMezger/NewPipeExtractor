...
public class YoutubeParsingHelper {
    ...
    public static TrackMetadata parseYtMusicAutoGenDescription(String description) throws ParsingException {
        TrackMetadata trackMetadata = new TrackMetadata();

        // to preserve breaks while parsing html replace them with newlines
        ...
        boolean isYTMusicTrack = mat.find();

        if (isYTMusicTrack) {
            final String title = mat.group(1).trim();
            final String artist = mat.group(2);
            final String album = mat.group(3);
            final String year = mat.group(4);
            final String date = mat.group(5);
            final String cleanArtist = mat.group(7);

            trackMetadata.setTitle(title);
            trackMetadata.setArtist(Optional.ofNullable(cleanArtist).orElseGet(() ->
                    Arrays.stream(artist.split("·")).map(String::trim).collect(Collectors.joining(", "))));
            trackMetadata.setAlbum(album);

            if (!isNullOrEmpty(date)) {
                DateWrapper dateWrapper = new DateWrapper(YoutubeParsingHelper.parseDateFrom(date));
                trackMetadata.setReleaseDate(dateWrapper);
            }

            if (!trackMetadata.isEmpty()) {
                return trackMetadata;
            } else {
                throw new ParsingException("Could not get music track metadata from YT Music auto-generated description");
            }
        } else {
            throw new ParsingException("Could not get music track metadata from given description");
        }
    }
    ...
    public static List<TrackMetadata> getTracksFromMusicInThisVideoSection(JsonArray metadataRows) throws ParsingException {
        ...
        for (final Object metadataRow : metadataRows) {
            final JsonObject metadataRowRenderer = ((JsonObject) metadataRow).getObject("metadataRowRenderer");

            final String key = YoutubeParsingHelper.getTextFromObject(metadataRowRenderer.getObject("title"));
            final String value = getTextFromObject((JsonObject) metadataRowRenderer.getArray("contents").get(0));

            if (!isNullOrEmpty(key) && !isNullOrEmpty(value)){
                switch (key){
                    case "Song": trackMetadata.setTitle(value); break;
        ...
        if (!metadataRows.isEmpty() && !trackMetadata.isEmpty()) {  // last Track
            musicTracks.add(trackMetadata);
        }
        return musicTracks;
    }
